...
@section('content')
    <div class="mb-8">
        <a href="{{ route('produk.index') }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-indigo-600 transition mb-4">
            <i data-lucide="arrow-left" class="w-4 h-4"></i>
            Kembali
        </a>
        <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Edit Produk</h1>
        <p class="mt-1 text-sm text-slate-500">Ubah data produk</p>
    </div>

    <div class="max-w-xl">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <form action="{{ route('produk.update', $produk->id) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Kode Produk <span class="text-slate-400 font-normal">(opsional)</span></label>
